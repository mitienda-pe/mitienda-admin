      ...
      <Card>
        <template #content>
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                <h3 class="font-semibold" :class="isConfigured ? 'text-green-800' : 'text-yellow-800'">
                  {{ isConfigured ? 'PayPal configurado' : 'PayPal no configurado' }}
                </h3>
              </div>
            </div>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            <div>
              <h3 class="text-lg font-semibold text-secondary-800 mb-4">Credenciales de PayPal</h3>
              <div class="space-y-4">
                <div>
                  <label class="block text-sm font-medium text-secondary-700 mb-2">Email de PayPal Business <span class="text-red-500">*</span></label>
                  <InputText v-model="formData.merchant_email" type="email" placeholder="tu-email@empresa.com" class="w-full" />
                  <small class="text-secondary-600">Debe ser una cuenta PayPal Business</small>
                </div>
              </div>
            </div>
      ...
      <Card>
        ...
        <template #content>
          <div class="space-y-4 text-sm">
            <p class="text-secondary-600">PayPal permite a tus clientes pagar con su cuenta PayPal o tarjetas internacionales.</p>
            <Divider />
            <div>
              <h4 class="font-semibold mb-2">Requisitos</h4>
              <ul class="list-disc list-inside text-secondary-600 space-y-1">
                <li>Cuenta PayPal Business</li>
                <li>Verificación de identidad</li>
                <li>IPN configurado</li>
              </ul>
            </div>
            <Divider />
            <Button label="Ir a PayPal" icon="pi pi-external-link" link class="w-full" @click="openPaypal" />
          </div>
        </template>
      </Card>
    </div>
...
watch(() => store.currentConfig, (config) => {
  if (config?.credentials) {
    ...
    formData.merchant_email = c.merchant_email ?? ''
...
async function handleSubmit() {
  if (!formData.merchant_email) {
    toast.add({ severity: 'warn', summary: 'Email requerido', life: 3000 })
    return
  }
