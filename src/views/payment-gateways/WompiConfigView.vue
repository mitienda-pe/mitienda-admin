      ...
      <Card>
        <template #content>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                >
                  {{ store.currentConfig.gateway.configured ? 'Wompi configurado' : 'Wompi no configurado' }}
                </h3>
                <p
                  class="text-sm mt-1"
                ...
                >
                  {{ store.currentConfig.gateway.configured
                    ...
                    : 'Configura tus credenciales de Wompi para empezar a recibir pagos en Colombia.'
                  }}
                </p>
              </div>
            </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            <div>
              <h3 class="text-lg font-semibold text-secondary-800 mb-4">Credenciales de Wompi</h3>
              <div class="space-y-4">
                <div>
                  ...
                  <InputText
                    ...
                    placeholder="Ej: pub_prod_xxxxxxxxxxxxxxxx"
                    class="w-full"
                ...
                <div>
                  <label for="events_secret" class="block text-sm font-medium text-secondary-700 mb-2">
                    Events Secret <span class="text-red-500">*</span>
                  </label>
                  <Password
                    id="events_secret"
                    v-model="formData.events_secret"
                    placeholder="Tu Events Secret (para webhooks)"
                    class="w-full"
                    :class="{ 'p-invalid': errors.events_secret }"
                    :feedback="false"
                    toggleMask
                  />
                  <small v-if="errors.events_secret" class="text-red-500">{{ errors.events_secret }}</small>
                </div>
              </div>
            </div>
            ...
            <div v-if="webhookUrl">
              ...
              <div class="bg-gray-100 p-4 rounded-lg">
                <p class="text-sm text-secondary-700 mb-2">Configura este URL en tu panel de Wompi:</p>
      ...
      <Card>
        ...
        <template #content>
          <div class="space-y-4 text-sm">
            <div>
              <h4 class="font-semibold text-secondary-800 mb-2">¿Qué es Wompi?</h4>
              <p class="text-secondary-600">
                Wompi es la pasarela de pagos de Bancolombia. Permite aceptar pagos con
                tarjetas, PSE, Nequi y transferencias Bancolombia en Colombia.
              </p>
            </div>
            <Divider />
            <div>
              ...
              <ol class="list-decimal list-inside space-y-2 text-secondary-600">
                <li>Regístrate en Wompi</li>
                <li>Accede al Dashboard de desarrolladores</li>
                <li>Copia tus llaves pública, privada y el Events Secret</li>
                <li>Pega las credenciales aquí</li>
              </ol>
            </div>
            <Divider />
            <div>
              <Button label="Ir a Wompi" icon="pi pi-external-link" link class="w-full" @click="openExternal('https://wompi.co/')" />
            </div>
          </div>
        </template>
      </Card>
...
const GATEWAY_CODE = 'wompi'

interface WompiFormData {
  public_key: string
  private_key: string
...
const formData = reactive<WompiFormData>({
  public_key: '',
  private_key: '',
  events_secret: '',
  environment: 'prueba',
...
onMounted(async () => {
  ...
  if (store.currentConfig?.credentials) {
    const creds = store.currentConfig.credentials.credentials as unknown as WompiFormData
    if (creds) {
...
function validateForm(): boolean {
  ...
  if (!formData.events_secret?.trim()) { errors.events_secret = 'El Events Secret es requerido'; valid = false }
  return valid
}
...
function handleDelete() {
  confirm.require({
    message: '¿Estás seguro de eliminar las credenciales de Wompi?',
    ...
    accept: async () => {
      ...
      if (result.success) {
        ...
        Object.assign(formData, { public_key: '', private_key: '', events_secret: '', environment: 'prueba' })
      }
    },
  })
}
