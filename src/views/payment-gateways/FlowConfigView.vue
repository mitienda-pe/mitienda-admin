      ...
      <Card>
        <template #content>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                >
                  {{ store.currentConfig.gateway.configured ? 'Flow configurado' : 'Flow no configurado' }}
                </h3>
                <p
                  class="text-sm mt-1"
                ...
                >
                  {{ store.currentConfig.gateway.configured
                    ...
                    : 'Configura tus credenciales de Flow para empezar a recibir pagos en Chile.'
                  }}
                </p>
              </div>
            </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            <div>
              <h3 class="text-lg font-semibold text-secondary-800 mb-4">Credenciales de Flow</h3>
              <div class="space-y-4">
                <div>
            ...
            <div v-if="webhookUrl">
              ...
              <div class="bg-gray-100 p-4 rounded-lg">
                <p class="text-sm text-secondary-700 mb-2">Configura este URL en tu panel de Flow:</p>
      ...
      <Card>
        ...
        <template #content>
          <div class="space-y-4 text-sm">
            <div>
              <h4 class="font-semibold text-secondary-800 mb-2">¿Qué es Flow?</h4>
              <p class="text-secondary-600">
                Flow es una plataforma chilena de pagos online. Permite aceptar pagos con
                tarjetas, transferencias bancarias y otros métodos populares en Chile.
              </p>
            </div>
            <Divider />
            <div>
              ...
              <ol class="list-decimal list-inside space-y-2 text-secondary-600">
                <li>Regístrate como comercio en Flow</li>
                <li>Accede a tu panel de comercio</li>
                <li>Ve a la sección de API y obtén tus llaves</li>
                <li>Pega las credenciales aquí</li>
              </ol>
            </div>
            <Divider />
            <div>
              <Button label="Ir a Flow" icon="pi pi-external-link" link class="w-full" @click="openExternal('https://www.flow.cl/')" />
            </div>
          </div>
        </template>
      </Card>
...
const formData = reactive<FlowFormData>({
  api_key: '',
  secret_key: '',
  environment: 'prueba',
})
...
onMounted(async () => {
  ...
  if (store.currentConfig?.credentials) {
    const creds = store.currentConfig.credentials.credentials as unknown as FlowFormData
    if (creds) {
...
function handleDelete() {
  confirm.require({
    message: '¿Estás seguro de eliminar las credenciales de Flow?',
    ...
    accept: async () => {
      ...
      if (result.success) {
        ...
        Object.assign(formData, { api_key: '', secret_key: '', environment: 'prueba' })
      }
    },
  })
}
