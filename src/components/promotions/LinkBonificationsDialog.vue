...
async function handleLinkProducts() {
  ...
  try {
    ...
    const productIds = selectedProducts.value.map(p => ({
      producto_id: p.id,
      atributo_id: undefined // Para bonificaciones sin atributos específicos
    }))

    await promotionsStore.linkBonifications(props.promotionId, productIds)
...
<template>
  <Dialog
    ...
    :closable="true"
    :draggable="false"
    header="Vincular Productos de Bonificación"
  ...
  >
    <div class="space-y-4">
      ...
      <div v-if="products.length > 0" class="border rounded-lg">
        ...
        >
          ...
          <Column field="price" header="Precio">
            <template #body="slotProps">
              {{ currencySymbol }} {{ slotProps.data.price }}
            </template>
          </Column>
          <Column header="Stock">
            <template #body="slotProps">
      ...
      <div v-else-if="!searchQuery" class="text-center py-8 text-gray-500">
        <i class="pi pi-gift text-4xl mb-2"></i>
        <p>Busca productos para vincular como bonificación</p>
        <p class="text-sm">Estos productos se regalarán cuando se cumpla la condición</p>
      </div>
